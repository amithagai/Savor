...
import { ACESFilmicToneMapping, Box3, Plane, Vector3 } from 'three'
import {
  buildCabinetLayout,
...
type Props = {
  ...
  onPositionsChange: (updates: CabinetPositions) => void
  ...
  const cloned = useMemo(() => {
    ...
    clone.traverse(node => {
      ...
      if (mesh.isMesh) {
        ...
        // Cabinet materials do not need to receive dynamic shadows from every
        // neighboring cabinet. The floor and wall still receive their shadows.
...
function Cabinet({ x, width, spec, modelUrl, active, dragHandlers }: {
  x: number
  width: number
  spec: CategorySpec
  modelUrl?: string
...
function UploadedFaucet({ x, width, modelUrl, dragHandlers }: {
  x: number
  width: number
  modelUrl?: string
  dragHandlers?: DragHandlers
...
function ConfiguratorScene({ layout, faucetItems = [], wallLengthCm, onPositionsChange, accessories, onAccessoryPositionChange, interactionMode }: SceneProps) {
  ...
  // Keep the physical wall at the exact requested length. The wider design
  // span only frames cabinets that already overflow it; it must never be used
  // as the wall geometry itself.
  ...
  return (
    <>
      ...
      <mesh position={[offsetX + wallWidthM / 2, WALL_HEIGHT_CM * CM / 2, -0.03]} receiveShadow>
